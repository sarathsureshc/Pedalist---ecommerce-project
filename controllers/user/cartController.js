...
const getCart = async (req, res) => {
  try {
    ...
    cart.items.forEach((item) => {
      ...
      if (bestOffer) {
        totalPrice +=
          (product.price - bestOffer.effectiveDiscount) * item.quantity;

        if (
          bestOffer.offerName &&
          offers.some(
            (o) =>
              o.offerGroup === "Global" && o.offerName === bestOffer.offerName,
          )
        ) {
          globalDiscount = Math.max(
            globalDiscount,
            bestOffer.effectiveDiscount,
          );
        }
      } else {
    ...
    const minPurchaseAmount = Math.min(
      ...offers
        .filter((o) => o.offerGroup === "Global")
        .map((o) => o.minPurchaseAmount || 0),
    );
    ...
    res.render("cart", {
      ...
      offer:
        globalDiscount > 0
          ? `Global Offer Applied: ₹${globalDiscount.toFixed(2)}`
          : "No offer",
