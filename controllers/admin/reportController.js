...
const getSalesReport = async (req, res) => {
    ...
    if (req.xhr) {
        return res.json({ salesData, totalSalesCount, totalOrderAmount, totalDiscount, topProduct });
    }
...
const downloadSalesReportPDF = async (req, res) => {
    try {
        ...
        const totalSalesCount = salesData.reduce((acc, sale) => acc + sale.totalSalesCount, 0);
        const totalOrderAmount = salesData.reduce((acc, sale) => acc + sale.totalRevenue, 0);
        const totalDiscount = salesData.reduce((acc, sale) => acc + (sale.productDiscount || 0), 0);
        ...
        // Sales Date Range
        let salesDateRange = filters.filterType === 'custom' 
        ...
        salesData.forEach((sale, index) => {
            ...
            doc.moveTo(10, rowY + 20).lineTo(10 + tableWidth, rowY + 20).stroke(); // Row border
        });

        // Draw the final bottom border of the table
        const lastRowY = tableTop + 40 + (salesData.length * 20);
        doc.moveTo(10, lastRowY).lineTo(10 + tableWidth, lastRowY).stroke();
...
const downloadSalesReportExcel = async (req, res) => {
    try {
        ...
        // Sales Date Range
        const salesDateRange = filters.filterType === 'custom' 
            ? `Sales Date: ${new Date(filters.startDate).toLocaleDateString()} - ${new Date(filters.endDate).toLocaleDateString()}`
            : `Sales Date: ${filters.filterType.charAt(0).toUpperCase() + filters.filterType.slice(1)}`;
